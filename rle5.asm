...
; Gray Defender
; 05/30/2020
; Program to Decrypt basic RLE compression
;********************************************************
...
RLE_NUM         ; Not used For Interleaved just need the label defined
               jmp            SUB_Interleaved  ; Jump to the interlvd routine if directed
endif
...
Non_Interleaved                 
               ...
               SetUpVars      RLE_NUM,Get_Data     ; Set up pointer to RLE_NUM, #of bytes to repeat
               SetUpVars      RLE_Char,Next_Ch     ; Point to REL_CHAR - Loads character to store on screen           
               SetUpVars      VIC_SCREEN,VIC_LOOP  ; Pointer to scn data at $400 and later $d800
               ...
               SetUpVars      RLE_Color,Next_Ch  ; Set up pointer to RLE_Color Label
               SetUpVars      RLE_CNUM,Get_Data  ; Set up pointer to RLE_CNUM, # bytes to repeat
               SetUpVars      VIC_COLOR,VIC_LOOP ; Set up pointer to point to COLOR data             
...
SUB_Interleaved     
               ...
               sta            ADD_AMT2+1           ; 1 for Non.  
Operator Calc 
               SetUpVars      RLE_Char+1,Get_Data  ; Set up Pointer to RLE_Char+1 to get length #
Operator Hilo                                      
               SetUpVars      RLE_Char,Next_Ch     ; Set up pointer to RLE_Char preparing next char to grab
               SetUpVars      VIC_SCREEN,VIC_LOOP  ;Pointer to scn data $400 and later $d800
               jsr            Draw_Screen          ; Draw the Screen characters only - no color 
               ...
               sta            Max_HIGHBYTE+1       ; exiting program at the end of screen               
Operator Calc  
               SetUpVars      RLE_Color+1,Get_Data ; Set up point to RLE_Color+1 to grab color
               SetUpVars      RLE_COLOR,Next_Ch    ; Set up pointer to RLE_Color preparing next color to grab
Operator Hilo
               SetUpVars      VIC_Color,VIC_LOOP   ; Set up pointer to point to COLOR data at $d800                                                       
               jsr            Draw_Screen          ; Draw the Screen color information only
               rts
...
Get_Data       lda            $FFFF          ;Self modified, Load RLE_NUM or (REL_Char+1) 
